...
static uint8_t cs_pin = -1;
static uint8_t rst_pin = -1;
...
{
    uint8_t readValue[4];
    uint8_t writeValue[6];
    memset(writeValue, 0x00, sizeof writeValue);
    writeValue[0] = address;
    writeValue[1] = value;
    spi_send_and_receive(writeValue, 2, readValue, 0, &cs_pin);
}
...
uint8_t SPI1_ReadReg(uint8_t address)
{
    ...
    memset(writeValue, 0xFF, sizeof writeValue);
    writeValue[0] = address;
    spi_send_and_receive(writeValue, 1, readValue, 4, &cs_pin);
    return readValue[1];
}
...
void MFRC522_Init(uint8_t rst, uint8_t cs)
{
    cs_pin = cs;
    gpio_reset_pin(cs_pin);         //configures the IOMUX for this pin to the GPIO function
    gpio_set_direction(cs_pin, GPIO_MODE_OUTPUT);
    gpio_set_level(cs_pin, 1);

    rst_pin = rst;
    gpio_reset_pin(rst_pin);         //configures the IOMUX for this pin to the GPIO function
    gpio_set_direction(rst_pin, GPIO_MODE_OUTPUT);
    gpio_set_level(rst_pin, 1);

    MFRC522_Reset();
    // Address format: 0XXXXXX0
    // value ->  0x0F page 72/109 -reset everything. 
